...
def train_cnn_optuna_step(
    ...
    study_name: str = "cnn_optimization_hybrid"
) -> Tuple[
    Annotated[pd.DataFrame, ArtifactConfig(name="cnn_results")],
    ...
    """
    logger.info("Starting CNN + Optuna training step")
    logger.info(f"Train data shape: {train_data.shape}")
    logger.info(f"Test data shape: {test_data.shape}")
    logger.info(f"Optuna study name: {study_name}, n_trials: {n_trials}")
    logger.info(f"CNN parameters: sequence_length={sequence_length}, forecast_horizon={forecast_horizon}")
    
    # Call the CNN training function directly and unpack the results
    ...
    # Unpack the tuple and return individual components for ZenML
    (cnn_results, best_hyperparameters_json, trained_model, 
     residuals, test_predictions, forecast_comparison) = results
    
    return (cnn_results, best_hyperparameters_json, trained_model,
            residuals, test_predictions, forecast_comparison)
...
@pipeline
def book_sales_cnn_lstm_pipeline(
    output_dir: str,
    selected_isbns: List[str] = None,
    column_name: str = 'Volume',
    split_size: int = 32,
    ...
    """
    Complete book sales data processing and CNN+LSTM hybrid modeling pipeline.
    ...
    5. Prepares data for CNN modeling
    6. Trains CNN models with Optuna optimization
    7. Generates residuals for LSTM hybrid approach
    8. Returns comprehensive results including model performance metrics
    """
    logger.info("Running complete book sales CNN+LSTM pipeline")
    ...
    # Train CNN models with Optuna optimization (now handles 6 outputs)
    cnn_results, best_hyperparameters_json, trained_model, residuals, test_predictions, forecast_comparison = (
        train_cnn_optuna_step(
            train_data=train_data,
            test_data=test_data,
            output_dir=output_dir,
            n_trials=n_trials,
            sequence_length=sequence_length,
            forecast_horizon=forecast_horizon,
            study_name="cnn_optimization_hybrid",
        )
    )
...
if __name__ == "__main__":
    ...
    # Define default parameters for CNN+LSTM hybrid modeling
    default_selected_isbns = [
        '9780722532935',  # The Alchemist
    ]

    # Run the complete CNN+LSTM hybrid pipeline with proper parameters
    results = book_sales_cnn_lstm_pipeline(
        output_dir=output_dir,
    ...
    print("Complete CNN+LSTM hybrid pipeline run submitted! Check the ZenML dashboard for outputs.")
