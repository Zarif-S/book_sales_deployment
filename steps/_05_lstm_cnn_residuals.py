...
def load_cnn_residuals_from_csv(data_dir: str = "outputs") -> pd.DataFrame:
    """
    Load residuals from CSV file saved by the CNN pipeline.
    """
    print("🔍 Loading CNN residuals from CSV file...")

    # Look for residuals CSV in the organized outputs directory structure
    organized_residuals_path = os.path.join(data_dir, "data", "residuals", "cnn_residuals.csv")
    ...
    if os.path.exists(organized_residuals_path):
        ...
        alternative_paths = [
            os.path.join(data_dir, "cnn_residuals.csv"),  # Direct in outputs
            "data/processed/cnn_residuals.csv",           # In data/processed
            "cnn_standalone_outputs/cnn_residuals.csv",   # Old standalone dir
...
def prepare_residuals_for_lstm_hybrid_training(
    ...
    """
    Prepare residuals data for hybrid CNN+LSTM training with correct data alignment.
    ...
    Args:
        residuals_df: DataFrame with residuals from CNN model
        cnn_forecasts: CNN forecast values for test period (should be 32 values)
    ...
    """
    print("🔧 Preparing hybrid CNN+LSTM data (FIXED - no duplication)")
    print(f"📊 Parameters: lookback={lookback}, forecast={forecast}")
    print(f"📊 Residuals shape: {residuals_df.shape}")
    print(f"📊 CNN forecasts shape: {cnn_forecasts.shape}")
    ...
    print(f"📊 CNN forecasts length: {len(cnn_forecasts)}")

    # Ensure we have exactly 32 forecasts for the test period
    if len(cnn_forecasts) != forecast:
        print(f"⚠️  Warning: Expected {forecast} forecasts, got {len(cnn_forecasts)}")
        # Truncate or pad as needed
        if len(cnn_forecasts) > forecast:
            cnn_forecasts = cnn_forecasts[:forecast]
        else:
            # Pad with last value if needed
            padding_length = forecast - len(cnn_forecasts)
            padding = np.full(padding_length, cnn_forecasts[-1])
            cnn_forecasts = np.concatenate([cnn_forecasts, padding])
        print(f"📊 Adjusted CNN forecasts length: {len(cnn_forecasts)}")

    # Step 2: Split residuals into train/test (matching the CNN model split)
    ...
    cnn_forecasts_scaled = scaler.transform(cnn_forecasts.reshape(-1, 1)).flatten()
    ...
    print(f"📊 CNN forecasts scaled stats - Mean: {cnn_forecasts_scaled.mean():.4f}, Std: {cnn_forecasts_scaled.std():.4f}")
    ...
    if len(train_residuals_scaled) >= lookback:
        ...
        # Use scaled CNN forecasts as test target
        Y_test = cnn_forecasts_scaled.reshape(1, -1)
    ...
    result_cnn_df.loc[result_cnn_df['data_type'] == 'test', 'cnn_forecast'] = cnn_forecasts
    result_cnn_df.loc[result_cnn_df['data_type'] == 'test', 'scaled_cnn_forecast'] = cnn_forecasts_scaled

    print(f"📊 Result dataframe shape: {result_cnn_df.shape}")
    print(f"📊 Training periods: {(result_cnn_df['data_type'] == 'train').sum()}")
    print(f"📊 Test periods: {(result_cnn_df['data_type'] == 'test').sum()}")
    ...
    return {
        ...
        'cnn_forecasts': cnn_forecasts,
        'result_cnn_df': result_cnn_df,
        'train_residuals_scaled': train_residuals_scaled,
        'test_residuals_scaled': test_residuals_scaled,
        'cnn_forecasts_scaled': cnn_forecasts_scaled
    }
...
def load_cnn_forecasts_from_csv(data_dir: str = "cnn_standalone_outputs") -> pd.DataFrame:
    ...
    """
    print("🔍 Loading CNN forecasts from CSV file...")

    # Look for CNN forecasts CSV in the data directory
    cnn_forecasts_csv_path = os.path.join(data_dir, "cnn_forecasts.csv")

    if not os.path.exists(cnn_forecasts_csv_path):
        print(f"⚠️  CNN forecasts CSV not found at: {cnn_forecasts_csv_path}")
        ...
        for alt_path in alternative_paths:
            if os.path.exists(alt_path):
                cnn_forecasts_csv_path = alt_path
                print(f"✅ Found CNN forecasts CSV at: {cnn_forecasts_csv_path}")
                break
        else:
            raise FileNotFoundError(f"CNN forecasts CSV not found. Expected at: {cnn_forecasts_csv_path}")

    try:
        cnn_forecasts_df = pd.read_csv(cnn_forecasts_csv_path)

        # Convert date column to datetime and set as index
        if 'date' in cnn_forecasts_df.columns:
            cnn_forecasts_df['date'] = pd.to_datetime(cnn_forecasts_df['date'])
            cnn_forecasts_df = cnn_forecasts_df.set_index('date')

        print(f"✅ Loaded CNN forecasts from CSV: {cnn_forecasts_csv_path}")
        print(f"📈 CNN forecasts shape: {cnn_forecasts_df.shape}")
        print(f"📅 Date range: {cnn_forecasts_df.index.min()} to {cnn_forecasts_df.index.max()}")
        print(f"📊 CNN forecasts stats - Mean: {cnn_forecasts_df['predicted'].mean():.4f}, Std: {cnn_forecasts_df['predicted'].std():.4f}")
        print(f"🔧 Model signature: {cnn_forecasts_df['model_signature'].iloc[0]}")

        return cnn_forecasts_df

    except Exception as e:
        print(f"⚠️  Could not load CNN forecasts CSV: {e}")
        raise ValueError(f"Failed to load CNN forecasts CSV: {e}")


def load_cnn_artifacts_from_zenml():
    """
    Load CNN residuals and test predictions from ZenML artifacts.
    """
    print("🔧 Loading CNN artifacts from ZenML...")

    try:
        from zenml.client import Client

        # Load CNN residuals from training period
        print("📋 Loading CNN residuals from ZenML artifacts...")
        # Note: You'll need to replace these artifact IDs with actual CNN artifact IDs
        # For now, we'll try to find them dynamically
        client = Client()
        
        # Try to find CNN residuals artifact
        try:
            # This is a placeholder - you'll need to get the actual artifact ID from your CNN pipeline run
            artifact1 = client.get_artifact_version("cnn_residuals_placeholder_id")
            residuals_data = artifact1.load()
            print("✅ Successfully loaded CNN residuals from ZenML")
        except:
            print("⚠️  Could not load CNN residuals from ZenML, will use CSV fallback")
            residuals_data = None

        # Try to find CNN forecasts artifact
        try:
            # This is a placeholder - you'll need to get the actual artifact ID from your CNN pipeline run
            artifact2 = client.get_artifact_version("cnn_forecasts_placeholder_id")
            forecasts_data = artifact2.load()
            print("✅ Successfully loaded CNN forecasts from ZenML")
        except:
            print("⚠️  Could not load CNN forecasts from ZenML, will use CSV fallback")
            forecasts_data = None
...
def complete_lstm_workflow(cnn_forecasts: np.ndarray = None):
    """
    Complete LSTM workflow: load data, train model, make predictions, and plot results.
    Now loads test data from CSV created by pipeline and trains LSTM on CNN residuals.

    Args:
        cnn_forecasts: CNN forecast values for hybrid approach (optional)
    """
    print("🚀 Starting Complete CNN+LSTM Workflow")
    print("=" * 70)
    ...
    try:
        ...
        # Load CNN residuals from CSV (required for LSTM training)
        print("📋 Loading CNN residuals...")
        try:
            residuals_df = load_cnn_residuals_from_csv()
            print("✅ Successfully loaded CNN residuals from CSV!")
        except Exception as e:
            print(f"❌ Failed to load CNN residuals: {e}")
            raise ValueError("❌ Could not load CNN residuals. Please run the CNN pipeline first.")
        ...
        # Load CNN forecasts from CSV (required for hybrid approach)
        print("📋 Loading CNN forecasts...")
        try:
            cnn_forecasts_df = load_cnn_forecasts_from_csv()
            print("✅ Successfully loaded CNN forecasts from CSV!")
            
            # Extract CNN forecasts from the DataFrame
            if 'predicted' in cnn_forecasts_df.columns:
                cnn_forecasts = cnn_forecasts_df['predicted'].values
                print(f"📋 Extracted {len(cnn_forecasts)} CNN forecasts from 'predicted' column")
                print(f"📊 CNN forecasts range: {cnn_forecasts.min():.2f} to {cnn_forecasts.max():.2f}")
                
                # Create test_predictions_df for compatibility
                test_predictions_df = cnn_forecasts_df.copy()
            else:
                raise ValueError("Cannot find 'predicted' column in CNN forecasts CSV")
                
        except Exception as e:
            print(f"❌ Failed to load CNN forecasts: {e}")
            raise ValueError("❌ Could not load CNN forecasts. Please run the CNN pipeline first.")

        # Use hybrid approach if we have forecasts, otherwise simple approach
        if cnn_forecasts is not None:
            print("📋 Using hybrid CNN+LSTM approach (notebook-style)")
            print(f"📊 CNN forecasts shape: {cnn_forecasts.shape}")
            ...
            raise ValueError("❌ CNN forecasts are required for hybrid approach. Cannot proceed without forecasts.")
        ...
        best_model, tuner, train_predictions, test_predictions = train_lstm_with_tuner(
            ...
            epochs=50,
            project_name='lstm_residuals_cnn_hybrid'
        )
        ...
        test_forecasts = lstm_data['cnn_forecasts']
        ...
        # Set cnn_forecast for plotting
        cnn_forecast = test_forecasts
        ...
        # Step 4: Create hybrid predictions by combining CNN + LSTM
        print("\n📋 Step 4: Creating hybrid CNN+LSTM predictions...")
        ...
        # Combine CNN forecasts with LSTM residual predictions
        # Hybrid = CNN_forecast + LSTM_residual_prediction
        hybrid_predictions = test_forecasts + lstm_test_predictions

        print(f"📊 Hybrid predictions created:")
        print(f"   • CNN forecasts: {test_forecasts[:3]} ... (mean: {test_forecasts.mean():.2f})")
        print(f"   • LSTM residual predictions: {lstm_test_predictions[:3]} ... (mean: {lstm_test_predictions.mean():.2f})")
        print(f"   • Hybrid (CNN + LSTM): {hybrid_predictions[:3]} ... (mean: {hybrid_predictions.mean():.2f})")
        ...
        cnn_mae = mean_absolute_error(test_series.values, test_forecasts)
        cnn_rmse = np.sqrt(mean_squared_error(test_series.values, test_forecasts))
        cnn_mape = np.mean(np.abs((test_series.values - test_forecasts) / test_series.values)) * 100
        ...
        print(f"   • CNN alone - MAE: {cnn_mae:.2f}, RMSE: {cnn_rmse:.2f}, MAPE: {cnn_mape:.2f}%")
        print(f"   • Hybrid CNN+LSTM - MAE: {hybrid_mae:.2f}, RMSE: {hybrid_rmse:.2f}, MAPE: {hybrid_mape:.2f}%")

        # Calculate improvement percentages with division by zero protection
        if cnn_mae > 0:
            improvement_mae = ((cnn_mae - hybrid_mae) / cnn_mae) * 100
        else:
            improvement_mae = 0.0
            
        if cnn_rmse > 0:
            improvement_rmse = ((cnn_rmse - hybrid_rmse) / cnn_rmse) * 100
        else:
        ...
        try:
            ...
            plotting_results = comprehensive_model_evaluation(
                ...
                model_signature="CNN_plus_LSTM_Hybrid_Book_Sales_Forecast",
                save_plots=True
            )
        ...
        hybrid_forecast = results['hybrid_forecast']  # Hybrid predictions (CNN + LSTM)
        cnn_forecast = test_forecasts  # CNN forecasts
        ...
        print(f"   • cnn_forecast shape: {np.array(cnn_forecast).shape}")
        ...
        cnn_forecast = np.array(cnn_forecast).flatten()
        ...
        print(f"   • cnn_forecast shape: {cnn_forecast.shape}")
        print(f"   • lstm_forecast shape: {lstm_forecast.shape}")
        print(f"   • artifact_data_values shape: {artifact_data_values.shape}")

        # Ensure all arrays have the same length
        min_length = min(len(test_data), len(hybrid_forecast), len(cnn_forecast), len(lstm_forecast), len(artifact_data_values))
        ...
        comparison_df = pd.DataFrame({
            ...
            'Hybrid CNN + LSTM': hybrid_forecast[:min_length],
            'CNN': cnn_forecast[:min_length],
        ...
        print("\n" + "=" * 70)
        print("🎉 COMPLETE HYBRID CNN+LSTM WORKFLOW RESULTS:")
        print("=" * 70)
        print(f"📊 Data Summary:")
        print(f"   • CNN residuals data points: {len(residuals_df)}")
        ...
        print(f"\n📁 Data Sources:")
        print(f"   • CNN residuals: cnn_residuals.csv")
        print(f"   • Test dataset: combined_test_data.csv")
        print(f"   • CNN forecasts: ZenML artifacts or fallback")
        print(f"   • Final hybrid predictions: CNN + LSTM residual corrections")
