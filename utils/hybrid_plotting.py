...
from steps._05_lstm_sarima_residuals import load_arima_residuals_from_csv, prepare_residuals_for_lstm_hybrid_training
...
def create_hybrid_forecast(
    ...
    # DIAGNOSTIC: Check if residuals are reasonable
    print(f"📊 First model forecast stats - Mean: {first_model_forecast.mean():.2f}, Std: {first_model_forecast.std():.2f}")
    print(f"📊 LSTM residuals stats - Mean: {lstm_residuals_prediction.mean():.2f}, Std: {lstm_residuals_prediction.std():.2f}")
    print(f"📊 Sample first model forecast: {first_model_forecast[:5]}")
    print(f"📊 Sample LSTM residuals: {lstm_residuals_prediction[:5]}")
    
    # Check residuals magnitude relative to forecast
    residuals_to_forecast_ratio = np.abs(lstm_residuals_prediction).mean() / np.abs(first_model_forecast).mean()
    print(f"⚠️  Residuals-to-Forecast ratio: {residuals_to_forecast_ratio:.3f} (should be small, <0.1 ideal)")
...
def comprehensive_model_evaluation(
    ...
    Args:
        series_train: Training data series
        series_test: Test data series (can be first model forecast)
        ...
        actual_test_data: Actual ground truth test data for evaluation (optional)
    ...
    # Determine evaluation target (use actual test data if provided, otherwise series_test)
    evaluation_target = actual_test_data if actual_test_data is not None else series_test
    print(f"📊 Using evaluation target: {'Actual test data' if actual_test_data is not None else 'Series test data'}")
    
    # Create comparison DataFrame
    comparison_df = create_comparison_dataframe(
        actual_data=evaluation_target.values,
        hybrid_forecast=hybrid_forecast,
        first_model_forecast=first_model_forecast[:len(evaluation_target)] if first_model_forecast is not None else None,
        lstm_residuals=test_pred_residuals[:len(evaluation_target)],
    ...
    if first_model_forecast is not None:
        first_model_metrics = calculate_metrics(evaluation_target.values, first_model_forecast[:len(evaluation_target)])
    ...
    results = {
        ...
        'hybrid_forecast': hybrid_forecast,
        'lstm_residuals_prediction': test_pred_residuals[:len(evaluation_target)],
        'figures': {
            'main_plot': fig_main
        },
